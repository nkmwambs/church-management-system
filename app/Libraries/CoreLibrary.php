...
class CoreLibrary {
    ...
    private function userPermissionControls(&$crud){
        ...
        if($stateInfo == 'list'){
            ...
            if(!$checkRoleHasDeletePermission) {$crud->unsetDelete();}
            if(!$checkRoleHasEditPermission) {$crud->unsetEdit();}
        }
    }
    ...
    private function fieldControlFunctions(&$crud, $featureLibrary){
        $fieldControlFunctions = [
            'columns','addFields','editFields','unsetAddFields','unsetEditFields','cloneFields','displayAs','unsetColumns',
            'unsetReadFields','where','requiredFields','unsetReadFields','uniqueFields','unsetCloneFields',
        ];
    ...
    private function callBacks(&$crud, $featureLibrary){
        ...
        $crud->callbackAfterInsert(function ($postObj) use ($featureLibrary) {
            if(method_exists($featureLibrary, 'callbackAfterInsert')){
                $postObj = $featureLibrary->callbackAfterInsert($postObj);
                return $postObj;
            }
        });
